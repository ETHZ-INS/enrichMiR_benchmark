...
# load splice SE
se <- readRDS("data/raw/jeong.raw.SE.rds")
```
...
# modify colData
se$miRNA <- factor(c("DKO","WT","WT","DKO"))
se$rep <- factor(1:2)
...
plgINS::plPCA(assays(se)$logcpm, as.data.frame(colData(se)), colorBy = "rep",
              add.labels = FALSE, annotation_columns = colnames(colData(se))[3])
```
...
se <- DEA(se, name="all", model = ~rep+miRNA, model0 = ~rep)
```
...
for(i in 1:length(mirna)){
  se <- DEA(se, use=se$miRNA %in% c(ctrl,mirna[i]), name=dea.names[i], model = ~rep+miRNA, model0 = ~rep)
}
```
...
se <- roundSE(se)
# export
saveRDS(se, "data/jeong.DEA.SE.rds")
```
