...
colData(se.splicing) <- cbind(colData(se.splicing), rbind(colData(se.hela), colData(se.hek))[colnames(se.splicing),1:3])
...
exonDEA <- function(se){
  ...
  ## add DEAs
  rowData(se)$DEA.spliced.all <- as.data.frame(res.comb)
  for(i in paste0("DEA.",dea.names)){
    rowData(se)[[i]] <- as.data.frame(res.list[[i]])
  }
...
# logFC heatmap HeLa
SEtools::sehm(se.hela, row.names(se.hela)[rowData(se.hela)$DEA.spliced.all$FDR < 0.01], gaps_at = "readtype",
              breaks=T, do.scale = T, show_colnames = TRUE, assayName = "log2FC")
# logFC heatmap HEK
SEtools::sehm(se.hek, row.names(se.hek)[rowData(se.hek)$DEA.spliced.all$FDR < 0.01], gaps_at = "readtype",
              breaks=T, do.scale = T, show_colnames = TRUE, assayName = "log2FC")
```
