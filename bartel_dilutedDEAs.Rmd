...
# AUC, approximated as the inverse of the number of positives to recover the true positives:
auc <- sapply(split(dat,dat$method), FUN=function(x){
    if(!any(x$TPR>=1)) return(0)
    1/x$P[which(x$TPR>=1)[1]]
})
```
