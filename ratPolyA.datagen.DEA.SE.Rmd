---
title: "ratPolyA DEA SE generation"
author: "tgermade"
date: "`r Sys.Date()`"
output:
  html_document:
...
# load splice SE
se <- readRDS("data/raw/ratPolyA.raw.SE.rds")
```
...
# modify colData
se$rep <- factor(1:4)
se$isBatch4 <- factor(grepl("_n4",colnames(se)))
se$miRNA <- factor(sapply(colnames(se), function(x) unlist(strsplit(x,"\\_"))[1]))
...
plgINS::plPCA(assays(se)$logcpm, as.data.frame(colData(se)), colorBy = "rep",
              add.labels = FALSE, annotation_columns = colnames(colData(se))[5])
```
...
dea.names <- paste0(unique(se$miRNA[se$miRNA!=ctrl]),"vNeg")
...
se <- DEA(se, name="all", model = ~isBatch4+miRNA, model0 = ~isBatch4)
```
...
for(i in 1:length(mirna)){
  se <- DEA(se, use=se$miRNA %in% c(ctrl,mirna[i]), name=dea.names[i], model = ~isBatch4+miRNA, model0 = ~isBatch4)
}
```
...
## add logFC assay
se <- plgINS::svacor(se, form = ~isBatch4+miRNA, form0 = ~isBatch4)
...
se <- roundSE(se)
# export
saveRDS(se, "data/ratPolyA.DEA.SE.rds")
```
...
### [model = ~isBatch4+miRNA, model0 = ~isBatch4]
