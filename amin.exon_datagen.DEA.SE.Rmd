---
title: "Amin enrichMiR exon analysis"
author: "tgermade"
date: "`r Sys.Date()`"
output:
    html_document:
...
# load splice SE
se.splicing <- readRDS("data/raw/amin.splicing.raw.SE.rds")
```
...
# modify colData
se.splicing$miRNA <- factor(c(rep("218DKO",2),rep("WT",2)))
se.splicing$rep <- factor(c(1,2,2,3))
...
plgINS::plPCA(assays(se)$logcpm, as.data.frame(colData(se)), colorBy = "rep", shapeBy = "readtype",
              add.labels = FALSE, annotation_columns = colnames(colData(se))[c(2,1,3)])
## spliced
...
se.comb <- roundSE(se.comb)
# export
saveRDS(se.comb, "data/amin.exon.DEA.SE.rds")
```
