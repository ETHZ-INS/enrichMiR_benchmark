...
#'
#' @examples
regmir.new <- function(e, sets, dea, binary=NULL, alpha=1, use.intercept=FALSE,
                   keepAll=TRUE, cons=TRUE, KO=TRUE, useFC=TRUE){
  ...
  res <- coef(summary(mod))
  res <- res[order(res[,4]),,drop=FALSE]
  colnames(res) <- c("beta","stderr",ifelse(binary,"z","t"),"pvalue")
  res <- res[grep("^\\(Intercept\\)$|FALSE$", row.names(res), invert=TRUE),,drop=FALSE]
  row.names(res) <- gsub("TRUE","",row.names(res))
...
# load
dea <- loadAll("../master_19_20/enrichMiR_benchmark/data/amin.DEA.SE.rds",TPs)
dea <- lapply(dea, cleanDEA)
e.list <- readRDS("../master_19_20/enrichMiR_benchmark/results/amin.enrichMiR.rds")
TS <- readRDS("../master_19_20/enrichMiR_benchmark/data/TargetScan_mouse.rds")
