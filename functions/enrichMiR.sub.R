...
enrichMiR <- function( DEA, TS, miRNA.expression=NULL, families=NULL, 
  ...
  if(is.null(tests) || "areamir.FC" %in% tests) o@res$aREAmir.FC=aREAmir(DEA, TS, minSize, pleiotropy=FALSE, useFC=TRUE) 
  if(is.null(tests) || "areamir.FDR" %in% tests) o@res$aREAmir.FDR=aREAmir(DEA, TS, minSize, pleiotropy=FALSE, useFC=FALSE) 
  
  if(is.null(tests) || "areamirp.FC" %in% tests) o@res$aREAmir2.FC=aREAmir(DEA, TS, minSize, pleiotropy=TRUE, useFC=TRUE) 
  if(is.null(tests) || "areamirp.FDR" %in% tests) o@res$aREAmir2.FDR=aREAmir(DEA, TS, minSize, pleiotropy=TRUE, useFC=FALSE) 
  
  TS <- as.data.frame(TS)
  if(is.null(tests) || "mw.FC" %in% tests) o@res$MW.FC=MW(DEA, TS, minSize, useFC=TRUE) 
  if(is.null(tests) || "mw.FDR" %in% tests) o@res$MW.FDR=MW(DEA, TS, minSize, useFC=FALSE) 
  
  if(is.null(tests) || "ks.FC" %in% tests) o@res$KS.FC=KS(DEA, TS, minSize, useFC=TRUE) 
  if(is.null(tests) || "ks.FDR" %in% tests) o@res$KS.FDR=KS(DEA, TS, minSize, useFC=FALSE) 
  
  if(is.null(tests) || "ks2.FC" %in% tests) o@res$KS2.FC=KS2(DEA, TS, minSize, useFC=TRUE) 
  if(is.null(tests) || "ks2.FDR" %in% tests) o@res$KS2.FDR=KS2(DEA, TS, minSize, useFC=FALSE) 
  
  if(is.null(tests) || "gsea.FC" %in% tests) o@res$GSEA.FC=gsea(DEA, TS, minSize, maxSize=gsea.maxSize, nperm=gsea.permutations, fdr.thres=gsea.fdr.thres, useFC=TRUE) 
  if(is.null(tests) || "gsea.FDR" %in% tests) o@res$GSEA.FDR=gsea(DEA, TS, minSize, maxSize=gsea.maxSize, nperm=gsea.permutations, fdr.thres=gsea.fdr.thres, useFC=FALSE) 
  
  if(is.null(tests) || "modsites.FC" %in% tests) o@res$modSites.FC=plMod(DEA, TS, minSize, var="sites", correctForLength=T, useFC=TRUE) 
  if(is.null(tests) || "modsites.FDR" %in% tests) o@res$modSites.FDR=plMod(DEA, TS, minSize, var="sites", correctForLength=T, useFC=FALSE) 
  
  if(is.null(tests) || "modscore.FC" %in% tests) o@res$modScore.FC=plMod(DEA, TS, minSize, var="score", correctForLength=F, useFC=TRUE) 
  if(is.null(tests) || "modscore.FDR" %in% tests) o@res$modScore.FDR=plMod(DEA, TS, minSize, var="score", correctForLength=F, useFC=FALSE) 
