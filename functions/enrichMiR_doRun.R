...
#'
#' @examples
getEnrichMiR <- function(dea.list, TS, TP, props, tests, mirexpr, cores=1){
  ### do TS permutations
  set.seed(1234)
  ...
  myf <- function(dea, TS){
    e <- enrichMiR(DEA=dea, TS=TS, miRNA.expression=mirexpr, cleanNames=FALSE, tests=tests, families=families)
  ...
  if(!is.null(mirexpr)){
    families <- metadata(TS)$families[intersect(names(metadata(TS)$families),names(mirexpr))]
  } else {
    families <- metadata(TS)$families
  }
  ...
  if(cores==1){
    e.list <- lapply(dea.list, FUN=function(dea){
      ...
      eres <- lapply(TSperm(TS, TP, rownames(dea), props), FUN=function(ts) myf(dea, ts))
      names(eres) <- names(props)
      c( list(original=enrichMiR(DEA=dea, TS=TS, miRNA.expression=mirexpr, cleanNames=FALSE, tests=tests, families=families)),
       eres )
    })
  }else{
    if(length(dea.list)==1){
      e.list <- lapply(dea.list, FUN=function(dea){
        dea <- as.data.frame(dea)
        eres <- bplapply(TSperm(TS, TP, rownames(dea), props), BPPARAM=MulticoreParam(cores, progressbar=TRUE),
...
allParams <- c(
  ...
  lapply(allds[9:14], FUN=function(x) 
    list(ds=x, ts="TargetScan_mouse.rds", miRNAs=NULL) )
)
...
mirParams <- c(
  ...
  lapply(allds[9:14], FUN=function(x) 
    list(ds=x, ts="TargetScan_mouse.rds", miRNAs=mirexpr$mouse) )
)


doRun <- function(p, cores=6, cleanRowNames=FALSE){
  ...
  dea.list <- getDEAList(se, dea.df.names, dea.names, cleanRowNames=cleanRowNames)
  ...
  eres <- getEnrichMiR( dea.list=dea.list, TS=TS, TP=TP, props=props, tests=tests,
                        mirexpr=p$miRNAs, cores=cores )
  ### summarize enrichMiR results in a dataframe
  bench <- getDF(dea.names, eres, TP)
  ### output
  if(is.null(p$miRNAs)){
    saveRDS(eres, file=file.path("results", gsub("DEA\\.SE","enrichMiR2",p$ds)))
    saveRDS(bench, file=file.path("results", gsub("DEA\\.SE","benchmark2",p$ds)))
  } else {
    saveRDS(eres, file=file.path("results", gsub("DEA\\.SE","mirexpr.enrichMiR2",p$ds)))
    saveRDS(bench, file=file.path("results", gsub("DEA\\.SE","mirexpr.benchmark2",p$ds)))
  }
  rm(eres)
