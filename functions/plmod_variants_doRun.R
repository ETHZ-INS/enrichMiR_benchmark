...
plmodRun <- function(p, pure=FALSE){
  ...
  for(w in cpm.weights <- c(T,F)){
    ...
    for(s in sig.bin <- c(T,F)){
      ...
      for(v in var <- c("sites","score")){
        print(paste("variable:", v))
        V=ifelse(v=="sites","bs","sc")
        cfl=ifelse(v=="sites",T,F)
          if(!pure){
            e <- lapply(names(dea), function(i) lapply(e.list[[i]]$original@res, function(e)
              plMod.new(e, TS, dea[[i]], KO=ko, cpm.weights=w, sig.binary=s, correctForLength=cfl, var=v)
            ))
            dir <- "results3/"
          } else {
            e <- lapply(names(dea), function(i) 
              plMod.new(e=NULL, TS, dea[[i]], KO=ko, cpm.weights=w, sig.binary=s, correctForLength=cfl, var=v, pure=TRUE)
            )
            dir <- "results3/plmod_pure/"
          }
          names(e) <- names(dea)
          saveRDS(e, paste0(dir, n, ".plmod.", S,"_",V,".",W,".rds") )
